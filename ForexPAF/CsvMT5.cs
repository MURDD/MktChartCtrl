...
namespace ForexPAF
{
    public class CsvMT5 : ICsvLoader
    {
        public static bool CanLoading(string filename)
        {
            using (var sr = new System.IO.StreamReader(filename))
            {
                ...
                if (values[1].IndexOf("TIME") < 0)
                    return false;

                if (values[2].IndexOf("OPEN") < 0)
                    return false;

                if (values[3].IndexOf("HIGH") < 0)
                    return false;

                if (values[4].IndexOf("LOW") < 0)
                    return false;

                if (values[5].IndexOf("CLOSE") < 0)
                    return false;

                if (values[6].IndexOf("TICKVOL") < 0)
                    return false;

                if (values[7].IndexOf("VOL") < 0)
                    return false;

                if (values[8].IndexOf("SPREAD") < 0)
                    return false;
            }
            return true;
        }
